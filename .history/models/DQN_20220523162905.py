...
def make_batch(memory, batch_size):
    ...
    obs_dicts = {'pixels': torch.tensor([], dtype=torch.float), 'timedelta': torch.tensor([])}
    next_obs_dicts = {'pixels': torch.tensor([], dtype=torch.float), 'timedelta': torch.tensor([])}
    actions = torch.tensor([], dtype=torch.int64)
    rewards = torch.tensor([], dtype=torch.float)
    dones = torch.tensor([], dtype=torch.float)
