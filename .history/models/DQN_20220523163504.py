...
def make_batch(memory, batch_size):
    ...
    obs_dicts = {'pixels': [], 'timedelta': torch.tensor([])}
    next_obs_dicts = {'pixels': [], 'timedelta': torch.tensor([])}
    actions = []
    rewards = []
    dones = []
    ...
    for action_dict in actions_dicts:
      torch.cat((actions, [action_dict['action_id']])
    ...
    for ts, next_ts in zip(time_steps, next_timesteps):
      ...
      torch.cat((obs_dicts['pixels'], obs_pixel))
      torch.cat((obs_dicts['timedelta'], [obs_timedelta]))
      torch.cat((next_obs_dicts['pixels'], next_obs_pixel))
      torch.cat((next_obs_dicts['timedelta'], [next_obs_timedelta]))
      
      torch.cat((rewards, [ts.reward]))
