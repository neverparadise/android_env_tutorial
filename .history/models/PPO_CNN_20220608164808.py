...
class PPO(nn.Module):
    def __init__(self, C, H, W, num_actions):
        ...
        self.fc_mu_x = nn.Linear(512, 1)
        self.fc_sigma_x = nn.Linear(512, 1)
        self.fc_mu_y = nn.Linear(512, 1)
        self.fc_sigma_y = nn.Linear(512, 1)
    ...
    def pi(self, obs):
        ...
        mu_x = torch.tanh(self.fc_mu_x(feature))
        sigma = F.softplus(self.fc_sigma_x(feature)) +1e-3
        return mu, sigma
