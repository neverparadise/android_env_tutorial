...
def make_batch(memory, batch_size):
    ...
    obs_dicts = {'pixels': np.array([]), 'timedelta': []}
    next_obs_dicts = {'pixels': np.array([]), 'timedelta': []}
    actions_list = []
    
    for ts, next_ts in zip(time_steps, next_timesteps):
      obs_pixel = converter(ts.observation)
      next_obs_pixel = converter(next_ts.observation)
      obs_dicts['pixels'].append()
      next_obs_dicts['pixels']
