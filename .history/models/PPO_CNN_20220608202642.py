...
def train_ppo(model, buffer, optimizer, K_epoch, lmbda, gamma, eps_clip, entropy_coef):
    ...
    for i in range(K_epoch):
        td_target = r + gamma * model.v(next_obs) * done_mask
        delta = td_target.detach() - model.v(obs)
        ...
        print(f'current_log_prob: {current_log_prob}')
        entropy = dist.entropy() * entropy_coef
        print(f'entropy: {entropy}')
        ratio = torch.exp(current_log_prob - old_log_prob)  # a/b == exp(log(a)-log(b))
        print(f'ratio: {ratio}')
        ...
        loss = (-torch.min(surr1, surr2) - entropy).mean() + critic_coef * F.smooth_l1_loss(current_value, td_target.detach())
