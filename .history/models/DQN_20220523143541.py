...
class ClassicCNN(nn.Module):
    ...
    def forward(self, batch_observations):
      pixels = batch_observations['pixels']
      time_deltas = batch_observations['time_delta']

      x = self.conv_module(pixels) 
      x = x.view(x.size(0), -1)
      cocat_x = torch.cat((x, time_deltas), dim=1)
      out = self.fc(cocat_x)

      return out
    
    def make_batch(self, observations):
      
    
    def converter(self, observation):
