...
from models.PPO_CNN import PPO, Buffer, train_net
from utils import save_model, load_model, converter, make_continuous_env, make_continuous_action
...
def main():
    ...
    print(H, W, C)
    model = PPO(C, H, n_actions).to(device).float()    # C, H, W, K, S, num_actions
    ...
    for episode in range(total_episodes):
        ...
        while not timestep.last():
            ...
            obs = converter(timestep.observation)

            # * 2. sample action from policy
            mu, sigma = model.pi(obs)
            dist = torch.distributions.Normal(mu,sigma)
            action_position = dist.sample() 
            log_prob = dist.log_prob(action)
            action = dict()
            action['action_type'] = 0
            action['touch_position'] = action_position.item()
            ...
            #model.put_data((s, action, r/10, s_prime, log_prob, done))
