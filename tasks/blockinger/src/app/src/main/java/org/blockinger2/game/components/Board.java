...
public class Board extends Component
{
    ...
    public void set(int x, int y, Square square)
    {
        ...
        if (RLTask.get().isEnabled()) {
            int[][] elements = new int[this.height][this.width];
            tempRow = topRow;
            for (int h = 0; h < height; h++) {
                if (tempRow != null) {
                    for (int w = 0 ; w < width ; w++) {
                        elements[h][w] = tempRow.get(w).isEmpty() ? 0 : 1;
                    }
                    tempRow = tempRow.getBelow();
                }
            }
            RLTask.get().logExtra("current_board " + Arrays.deepToString(elements));
        }
    }
