...
public class Controls extends Component
{
    ...
    {
        RLTask.get().logExtra("rotate_left_pressed [1]");
        leftRotation = true;
        host.game.action();
        vibrateShort();
    ...
    {
        RLTask.get().logExtra("rotate_right_pressed [1]");
        rightRotation = true;
        host.game.action();
        vibrateShort();
        host.sound.buttonSound();
    ...
    {
        RLTask.get().logExtra("down_pressed [1]");
        host.game.action();
        playerSoftDrop = true;
        clearPlayerSoftDrop = false;
        vibrateShort();
    ...
    {
        RLTask.get().logExtra("drop_pressed [1]");
    ...
    {
        RLTask.get().logExtra("left_pressed [1]");
        host.game.action();
        clearLeftMove = false;
        leftMove = true;
        rightMove = false;
    ...
    {
        RLTask.get().logExtra("right_pressed [1]");
        host.game.action();
        clearRightMove = false;
        rightMove = true;
        leftMove = false;
    ...
    public void cycle()
    {
        ...
        if (playerHardDrop) {
            ...
            RLTask.get().logExtra("current_line [" + active.getY() + "]");
            vibrateBottom();
